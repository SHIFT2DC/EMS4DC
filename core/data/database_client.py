...
class LastIntervalQuerier:
    ...
    def get_last_15min_averages(self, table_name: str = "measurements", 
        ...
        try:
            ...
            self.logger.debug(f"Data age: {age_minutes:.1f} minutes")
            ...
            self.logger.debug(f"Data is fresh (within {max_age_minutes} minutes)")
            self.logger.debug(f"Retrieved averages for {len(df)} parameters")
            return df
    ...
    def get_most_recent_values(self, table_name: str = "measurements",
        ...
        try:
            ...
            self.logger.debug(f"Oldest recent data age: {age_minutes:.1f} minutes")
            ...
            self.logger.debug(f"Data is fresh (within {max_age_minutes} minutes)")
            self.logger.debug(f"Retrieved most recent values for {len(df)} parameters")
            return df
