...
# Define your timezone once here
TIMEZONE = ZoneInfo("Europe/Berlin")  # UTC+1 (UTC+2 during DST)
...
def calculate_time_for_execution(
    interval_minutes: float = 15, start_time: datetime = None, now_time: datetime = None
):
    ...
    Args:
        ...
        now_time (datetime, optional): current time for running this function, which will decide the real sleeping
    ...
    now = start_time if start_time is not None else current_time()
    ...
    now_time = now_time if now_time else current_time()
    logger.debug(f"Current system time is {now_time}")

    sleep_time = (next_execution - now_time).total_seconds()
    if sleep_time < 0:
        sleep_time = 0
