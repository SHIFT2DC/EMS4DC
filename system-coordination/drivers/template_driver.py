...
class TemplateDriver(BaseDeviceDriver):
    ...
    def _load_device_config(self) -> Dict[str, Any]:
        ...
        # Extract Device X configuration from config.json file
        device_x_config = config.get('deviceX', {})
        
        return device_x_config
    ...
    def transform_droop_curve(self, optimizer_data: Dict[str, float], config_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        This is the template function which shows how a transformation can be written.
        Moreover, this template shows how the values from config.json can be extracted to use in this function.
        Outputs from optimizer are also used in this template.
        Below is example for AFE driver
