...
class ModbusWriter:
    ...
    def write_setpoints(self, device_name: str, setpoints: Dict[str, float]) -> Dict[str, bool]:
        """
        Write setpoints to a device
        
        Args:
            device_name: Name of the device (e.g., 'AFE', 'BESS')
        ...
        """
        self.logger.info(f"Writing {len(setpoints)} setpoints to {device_name}")
        
        results = {}
        for param_name, value in setpoints.items():
            try:
                success = self.reader.write_single_register(device_name, param_name, value)
        ...
        self.logger.info(f"Wrote {success_count}/{len(setpoints)} setpoints successfully")
    ...
    def write_device_setpoints_batch(self, device_setpoints: Dict[str, Dict[str, float]]) -> Dict[str, Dict[str, bool]]:
        ...
        for device_name, setpoints in device_setpoints.items():
            results[device_name] = self.write_setpoints(device_name, setpoints)
        return results
