...
class PVDriver(BaseDeviceDriver):
    ...
    def __init__(self, device_id: str, config: str):
        ...
        self.DEVICE_TYPE = "UNI_EV"
    
    def _load_device_config(self, asset_key: str = "uniev1") -> Dict[str, Any]:
        """
    ...
    def transform_droop_curve(self, optimizer_data: Dict[str, float], config_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        This is droop curve transformation for V1G EV charger
        
        Args:
            optimizer_data: Dictionary containing optimizer values
                - charge: Power setpoint to charge
                - soc: target soc for car to reach
        ...
        ev_charge = optimizer_data.get('charge') * 1000
        soc = optimizer_data.get('soc') * 10
        
        undersupply_lim_v = config.get('droopVoltageLowerLimit')
        active_power_ref_max = ev_charge * 1.1
        start_discharge_v = 690

        # Create the return dictionary with register names as keys
        droop_curve_registers = {
            "UNDERSUPPLY_LIM_V": undersupply_lim_v,
            "ACTIVE_P_REF_MAX": active_power_ref_max,
            "START_DISCHARGE_V": start_discharge_v
        }
